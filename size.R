...
pak_ratio %>% group_by(size, date) %>% 
  ...
  geom_bar(aes(fill = date), stat = "identity") + geom_smooth() +
  facet_grid(. ~size)
...
pak_ratio %>% group_by(size, date) %>% 
  ...
  geom_line()+geom_hline(aes(yintercept  = 0))+ theme_economist()+
  labs(y = "Cumulative Returns(%)",
   title = "Cumulative Returns of Size Portfolios of PSX")
...
Ret %>%  select(date, SMB, HML) %>% gather(factor, returns, -date) %>%
  ...
  geom_line(size = 1)+geom_hline(aes(yintercept  = 0))+ theme_economist()+
  labs(y = "Cumulative Returns(%)",
       title = "Cumulative Returns of Fama & French Factors in PSX ",
